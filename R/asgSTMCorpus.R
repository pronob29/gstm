#' gSTM Corpus Coercion
#'
#' Convert a set of document term counts and associated metadata to
#' the form required for processing by the \code{\link{gstm}} function.
#'
...
#' \code{"data"} in the form needed for further processing by the \code{gstm}
#' function.
#'
#' @seealso \code{\link{prepDocuments}}, \code{\link{gstm}}
#'
#' @examples
#' \donttest{
#' library(quanteda)
...
#'                      stem = TRUE)
#' asgSTMCorpus(gadarian_dfm)
#' }
#' @export
asgSTMCorpus <- function(documents, vocab, data = NULL, ...) {
  UseMethod("asgSTMCorpus")
}

#' @method asgSTMCorpus list
#' @export
#' @keywords internal
asgSTMCorpus.list <- function(documents, vocab=NULL, data = NULL, ...) {
  list(documents = documents, vocab = vocab, data = data)
}

#' @method asgSTMCorpus dfm
#' @export
#' @keywords internal
asgSTMCorpus.dfm <- function(documents, vocab, data = NULL, ...) {
  if (!missing(vocab)) {
  ...
  dfm_gstm <- quanteda::convert(documents, to = "gstm", docvars = data)
  if(is.null(data)) data <- dfm_gstm[["meta"]]

  list(documents = dfm_gstm[["documents"]], vocab = dfm_gstm[["vocab"]],
       data = data)
}

#' @method asgSTMCorpus dgCMatrix
#' @export
#' @importFrom methods as
#' @keywords internal
asgSTMCorpus.dgCMatrix <- function(documents, vocab, data = NULL, ...) {
  if (!missing(vocab)) {
    # in case K was not specified by name, and it was confused with the
    # vocab argument (missing for dfm inputs)
    if (is.numeric(vocab) & length(vocab)==1) {
...
#' @method asgSTMCorpus dgTMatrix
#' @export
#' @keywords internal
asgSTMCorpus.dgTMatrix <- function(documents, vocab, data = NULL, ...) {
    documents <- methods::as(documents,"dgCMatrix")
    asgSTMCorpus.dgCMatrix(documents, vocab, data, ...)
}
