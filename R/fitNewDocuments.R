...
#' \code{\link{gstm}} model fit you will not necessarily get exactly the same result.  \code{\link{gstm}}
...
#' The columns need to match the ordering of the design matrix for the original \code{gstm} object.  
#' The design matrix in an gstm model called \code{gstmobj} can be found in \code{gstmobj$settings$covariates$X} 
...
#' 
#' @param model the originally fit gSTM object.
#' @param documents the new documents to be fit. These documents must be in the gstm format and
...
#' @param origData the original metadata used to fit the gSTM object. 
#' @param prevalence the original formula passed to prevalence when \code{gstm} was called. The function
...
fitNewDocuments <- function(model=NULL, documents=NULL, newData=NULL, 
  ...
  #Some checks from the gstm() function
  ...
  if(is.null(model)) stop("Please specify a fitted gSTM object for model")
  if(is.null(documents)) stop("Please specify an gstm formatted document list for documents argument")
    ...
    if(!is.null(designMatrix)) {
      ...
      if(!is.null(prevalence)) {
        ...
        if(!is.null(model$setting$covariates$formula)) {
          #if its a new gstm object we've saved it.
