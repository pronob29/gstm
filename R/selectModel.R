#' Assists the user in selecting the best gSTM model.
#' 
...
#' exclusivity, and sparsity (based on default gSTM run using selected
...
#' @param runs Total number of gSTM runs used in the cast net stage.
#' Approximately 15 percent of these runs will be used for running a gSTM until
#' convergence.
...
#' (DMR), a random initialization or a previous gSTM object.
...
#' \code{multigSTM()} on a large number of output models.
#' @param \dots Additional options described in details of gstm.
#' @return \item{runout}{List of model outputs the user has to choose from.
#' Take the same form as the output from a gstm model.} \item{semcoh}{Semantic
...
selectModel <- function(documents, vocab, K,
  ...
  for(i in 1:runs){
    ...
    mod.out <- gstm(documents, vocab, K,
