...
gstm.control <- function(documents, vocab, settings, model=NULL) {
  ...
  if(is.null(model)) {
    ...
    #initialize
    model <- gstm.init(documents, settings)
    ...
    #extract from a standard gSTM object so we can simply continue.
    mu <- model$mu
  ...
  model <- list(mu=mu, sigma=sigma, beta=beta, settings=settings,
                ...
                invsigma=solve(sigma), time=time, version=utils::packageDescription("gstm")$Version)
