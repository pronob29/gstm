...
#' 
#' Computes the multinomial dispersion of the gSTM residuals as in Taddy (2012)
#' 
...
#' 
#' @param gstmobj An \code{gSTM} model object for which to compute residuals.
#' @param documents The documents corresponding to \code{gstmobj} as in
#' \code{\link{gstm}}.
...
#' mod.out <- gstm(docs, vocab, 3, prevalence=~treatment + s(pid_rep), data=meta,
#'                max.em.its=5)
#' checkResiduals(mod.out, docs)
#' @export
checkResiduals <- function(gstmobj, documents, tol=.01) {
  
  # Convert the corpus to the internal gSTM format
  args <- asgSTMCorpus(documents)
  documents <- args$documents

  beta <- lapply(gstmobj$beta$logbeta, exp)
  theta <- gstmobj$theta
  index <- gstmobj$settings$covariates$betaindex
  K <- gstmobj$settings$dim$K
  n <- length(documents)
  phat <- gstmobj$settings$dim$V
  d <- n*(K-1) + K*( phat-1 )
