...
#' \item{bound}{calculated by \code{\link{gstm}} accessible by \code{max(model$convergence$bound)}}
...
#' @param documents The documents to be used for the gstm model
#' @param vocab The vocabulary to be used for the gstmmodel
#' @param K A vector of different topic numbers
#' @param init.type The method of initialization. See \code{\link{gstm}} for
...
get_statistics <- function(k, heldout, init.type, M, ...) { # k = one particular topic number; K = vector of topic numbers
  ...
  out[['K']] <- k
  #run gstm
  model <- gstm(documents=heldout$documents,vocab=heldout$vocab,
