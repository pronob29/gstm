#' Semantic Coherence
#' 
#' Calculate semantic coherence (Mimno et al 2011) for an gSTM model.
#' 
...
#'
#' @param model the gSTM object
#' @param documents the gSTM formatted documents (see \code{\link{gstm}} for format).
...
semanticCoherence <- function(model, documents, M=10){
  if(!inherits(model, "gSTM")) stop("model must be an gSTM object")
