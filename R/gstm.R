#' Variational EM for the Group-specific Structural Topic Model
#'
#'
...
#' the package here: \code{\link{gstm-package}}
#'
#' This is the main function for estimating a Group-specific Structural Topic Model (gSTM).
#' gSTM is an admixture with covariates in both mixture components.  Users
...
#' \code{Dates} or other non-numeric, non-factor types will work in \code{gstm}
...
#' tuned for the gSTM but almost all the relevant arguments can be changed
...
#' seeing different answers on different machines, see https://github.com/pronob29/gstm
...
#'
#' Models can now be restarted by passing an \code{gSTM} object to the argument
...
#'
#' You can pass a custom initialization of the beta model parameters to \code{gstm}.
#'
#'
...
#'  method.  See https://github.com/pronob29/gstm for more discussion.}
...
#' formatted in the same way as the piece of a returned gstm model object \code{gstmobj$beta$logbeta}.
...
#' the returned form of \code{gstmobj$beta$logbeta} and ensure that it matches your format.}
...
#' parameter.  This defaults to 30 and can throw an error when too high.  \code{gstm} will automatically lower
...
#' in the native \pkg{gstm} format, a sparse term count matrix with one row
...
#' @param seed Seed for the random number generator. \code{gstm} saves the seed
...
#' @param model A prefit model object.  By passing an \code{gstm} object to this
...
#'
#' @return An object of class gSTM
#'
...
#'
#' @title gstm: Variational EM for the Group-Specific Structural Topic Model
#' @description The Group-Specific Structural Topic Model (gSTM) extends the
#' Structural Topic Model (STM) to allow researchers to estimate topic models
#' with document-level and group-specific covariates. This function includes
#' tools for model selection, visualization, and estimation of topic-covariate
#' regressions, which are particularly useful for studying structured document
#' collections where group dynamics are of interest.
...
#' set.seed(02138)
#' mod.out <- gstm(out$documents, out$vocab, 3,
...
#'                      stem = TRUE)
#' gstm_from_dfm <- gstm(gadarian_dfm, K = 3, prevalence = ~treatment + s(pid_rep),
#'                     data = docvars(gadarian_corpus))
#'
#' #An example of restarting a model
#' mod.out <- gstm(out$documents, out$vocab, 3, prevalence=~treatment + s(pid_rep),
#'                data=out$meta, max.em.its=5)
#' mod.out2 <- gstm(out$documents, out$vocab, 3, prevalence=~treatment + s(pid_rep),
#'                 data=out$meta, model=mod.out, max.em.its=10)
#' }
#' @export
gstm <- function(documents, vocab, K,
  ...
  # Convert the corpus to the internal gSTM format
  args <- asgSTMCorpus(documents, vocab, data)
  ...
  ###
  return(gstm.control(documents, vocab, settings,model))
}
