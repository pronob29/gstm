#' Estimates regressions using an gSTM object
#' 
#' Estimates a regression where documents are the units, the outcome is the
#' proportion of each document about a topic in an gSTM model and the covariates
#' are document-meta data.  This procedure incorporates measurement uncertainty
#' from the gSTM model using the method of composition.
#' 
...
#' @param gstmobj Model output from gSTM
...
#' namespace.  It will not look for them in the \code{gSTM} object which for
...
#' provide the documents object (see \code{\link{gstm}} for format).
...
estimateEffect <- function(formula,
  ...
  if(!is.null(documents)) {
    ...
    args <- asgSTMCorpus(documents, data=metadata)
  ...
  if(attr(termobj, "response")==1){
    ...
    if(!(posint(K) && max(K)<=gstmobj$settings$dim$K)) stop("Topics specified as response in formula must be a set of positive integers equal to or less than the number of topics in the model.")   
  ...
  for(i in 1:nsims) {
    # 3a) simulate theta
    if(thetatype=="None") thetasims <- gstmobj$theta
    else {
      thetasims <- thetaPosterior(gstmobj, nsims=1, type=thetatype, documents=documents)
