...
#' are the document-topic proportions in the \code{\link{gstm}} model along with
...
#' variables to included in the LASSO beyond the topics present in the gstmobj.
...
#' the metadata file used in the gstm analysis. It must have a number of rows
#' equal to the number of documents in the gstmobj.
#' @param gstmobj The gSTM object, and output from the \code{gstm} function.
...
#' gstm1 <- gstm(poliblog5k.docs, poliblog5k.voc, 50,
#'             prevalence=~rating + blog, data=poliblog5k.meta,
#'             init.type="Spectral")
#' #make a plot of the topics most predictive of "rating"
#' out <- topicLasso(rating ~ 1, family="binomial", data=poliblog5k.meta,gstmobj=gstm1)
#' #generate some in-sample predictions
#' pred <- predict(out, newx=gstm1$theta,type="class")
#' #check the accuracy of the predictions
#' table(pred, poliblog5k.meta$rating)
#' }
#' @export
topicLasso <- function(formula, data, gstmobj=NULL, subset=NULL,
