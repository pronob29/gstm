#########################################################################
#         Generalized Model Stability Analysis Routines for gSTM         #
#             Antonio Coppola, Harvard University, July 2014            #
#########################################################################

#' Analyze Stability of Local gSTM Modez
#' 
#' This function performs a suite of tests aimed at assessing the global
#' behavior of an gSTM model, which may have multiple modes. The function takes
#' in a collection of differently initialized gSTM fitted objects and selects a
...
#' 
#' @aliases multigSTM print.MultimodDiagnostic
...
#' the output from a gSTM model. Currently only works with models without
#' content covariates.
...
#' of two different gSTM fitted models. The alignment is performed by solving
...
#' proportion of each document about a topic in an gSTM model, and the
...
#' @param from.disk If set to \code{TRUE}, \code{multigSTM()} will load the
...
#' \code{multigSTM()} inspects the current directory for RData files.
...
#'                        data=meta, runs=20)
#' 
#' out <- multigSTM(mod.out, mass.threshold = .75, 
...
#' 
#' out <- multigSTM(from.disk=T, mass.threshold = .75, 
#'                 reg.formula = ~ treatment,
#'                 metadata = gadarian)
#' }
#' @export
multigSTM <- function(mod.out=NULL, ref.model=NULL, 
  ...
  if(length(mod.out$runout[[1]]$beta$logbeta)>1) 
    stop("multigSTM does not yet work with content covariates.")
...
#' are returned by the function \code{multigSTM()}, which performs a battery of
#' tests aimed at assessing the stability of the local modes of an gSTM model.
#' 
...
print.MultimodDiagnostic <- function(x,...){
  cat(paste("A multimodality diagnostic object for a collection of gSTM models.
