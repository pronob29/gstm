...
#' 
#' @param gstmobj The gSTM model object to be used in making the word cloud.
...
#' \code{\link{gstm}}.  This is only necessary if \code{type="documents"}.
...
#' @seealso \code{\link{plot.gSTM}}
#' @references Ian Fellows (2014). wordcloud: Word Clouds. R package version
#' 2.5.  \url{https://cran.r-project.org/package=wordcloud}
#' @examples
#' cloud(gadarianFit, 1)
#' @export
cloud <- function(gstmobj, topic=NULL, type=c("model", "documents"), documents, 
                  thresh=.9, max.words=100, ...) {
  if(!requireNamespace("wordcloud", quietly=TRUE)) {
    stop("wordcloud package required to use this function.")
  } else {
    if(!inherits(gstmobj,"gSTM")) stop("cloud function only works for gSTM models.  See wordcloud package for general tools.")
