...
#' @param documents the documents to be modeled (see \code{\link{gstm}} for format).
...
#' meta <- prep$meta
#' 
#' gstm1<- gstm(documents, vocab, 5,
#'            prevalence =~ rating+ s(day),
#'            init.type="Random",
#'            data=meta, max.em.its=5)
#' eval.heldout(gstm1, heldout$missing)
#' 
#' @export
make.heldout <- function(documents, vocab, N=floor(.1*length(documents)), 
  ...
  args <- asgSTMCorpus(documents, vocab)
