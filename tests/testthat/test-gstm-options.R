library(gstm)
context("gstm function options")

test_that("Test that CTM gstm works", {
  test <- gstm(poliblog5k.docs, poliblog5k.voc, K=3, max.em.its = 1, init.type = "Random")
  expect_true(inherits(test,"gSTM"))
})

test_that("Test that spectral init works", {
  test <- gstm(poliblog5k.docs, poliblog5k.voc, K=3, max.em.its = 1, init.type = "Spectral")
  expect_true(inherits(test, "gSTM"))
})

test_that("Test that Prevalence works", {
  test <- gstm(poliblog5k.docs, poliblog5k.voc, 
...
test_that("Test that ngroups works", {
  test <- gstm(poliblog5k.docs, poliblog5k.voc, K=3, init.type="Random",
